...
/// INFO ///////////////////////////////////////////////////////////////////////
//
// FourIndex.c: Four Index Monolitic Formulation
//
...
int FourIndex(CONFIGURATION param, INSTANCE data, RESULTS *results) {
	...
	lp = CPXcreateprob(env, &status, "FourIndex");
	assert(lp != NULL);
	...
	//status = CPXnewlongannotation (env, lp, "cpxBendersPartition", 0); 
	status = CPXsetintparam (env, CPXPARAM_Benders_Strategy, 3);
	CPXsetintparam(env,CPX_PARAM_MIPDISPLAY,3); //different levels of output displ
	//CPXsetdblparam(env,CPXPARAM_Benders_Tolerances_feasibilitycut,0.000000001);
	//CPXsetdblparam(env,CPXPARAM_Benders_Tolerances_optimalitycut,0.000000001);
	start = clock();
